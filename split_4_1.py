...
def sample_tp(instance_file):
    ...
    for instance in sorted_ins:
        ...
        body = parts[5]
        body = body[2:-2].split("', '")  # 去掉开头和结尾的方括号，并分割
        if m == 1:
            en1, rel, en2 = body[0].split("\\t")
            if rel.startswith('inv_'):
                tp2.add(f"{en2}\t{rel[4:]}\t{en1}")
            else:
                tp2.add(f"{en1}\t{rel}\t{en2}")
        else:
            en1, rel, en2 = body[1].split("\\t")
            if rel.startswith('inv_'):
                tp2.add(f"{en2}\t{rel[4:]}\t{en1}")
            else:
                tp2.add(f"{en1}\t{rel}\t{en2}")
...
def tp_new_ent(tp, entities_count=5408,top_n=3500, sample_num=620):
    ent = set()
    for line in tp:
    ...
    tp_random = random.sample(tp_remain, 62023  - len(keys_list))
